...
- **Deck authorizer (rule 100 legality):** `DeckManager.authorize_deck(deck_id, db)` / `authorize_deck_def(deck, db)` → error strings ([] = legal). Checks every card id resolves in the CardDatabase (catches unknown AND not-yet-implemented ids — the db only loads `engine_status=implemented`), no Hero cards among the 60 (100.1), max 4 copies per name (100.4), faction icons match the hero, and class icons share a class with the hero (100.2a). Class restriction is enforced ONLY on Abilities/Equipment — an ally's class column is its flavor subtype, never a deckbuilding restriction. Multi-class cards use concatenated two-letter abbrevs in the class column (`MaPrLo`; Lo=Warlock — `DeckManager.CLASS_ABBREVS`). Called by playtest `_launch_game` before the menu is torn down, so a hand-edited custom deck fails with readable menu errors instead of breaking mid-game; `test_deck_manager.gd` authorizes every shipped deck (this replaces composition-pinning asserts).
